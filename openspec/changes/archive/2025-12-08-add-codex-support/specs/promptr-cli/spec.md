...
- **THEN** the CLI creates a symlink at the target path for symlink-capable agents or copies all `*.md` files (excluding `README.md`) into the AntiGravity workflows directory, Roo commands directory, or Codex prompts directory with `promptr-` prefixed filenames for Codex, then records the linked target in config unless a conflicting or unsafe link exists

#### Scenario: Link all agents
- **WHEN** the user runs `promptr link all`
- **THEN** the CLI creates symlinks for OpenCode and Claude and copies the prompt `*.md` files (excluding `README.md`) into AntiGravity, Roo, and Codex (with `promptr-` prefixed filenames for Codex) using the same safety rules and records each successful target in config
...
- **THEN** the CLI replaces the existing asset after validating the path is not nested or self-referential (for symlinks), applies `promptr-` prefixes for Codex copies, and updates the config to reflect the link state
...
- **THEN** the CLI removes the symlink or deletes the copied prompt `*.md` files for AntiGravity, Roo, or Codex (removing only the `promptr-` prefixed files for Codex), clears the stored link state for that target, and reports completion
