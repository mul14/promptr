...
Promptr CLI SHALL provide `promptr link [--force] <agent|all>` to create symlinks from `~/.prompts` into supported agent locations and prevent nested or self-referential links.

#### Scenario: Link single agent
- **WHEN** the user runs `promptr link antigravity`
- **THEN** the CLI creates a symlink at `~/.gemini/antigravity/global_workflows/promptr` pointing to `~/.prompts` unless a conflicting or unsafe link exists

#### Scenario: Link all agents
- **WHEN** the user runs `promptr link all`
- **THEN** the CLI creates symlinks for AntiGravity and OpenCode targets from `~/.prompts` using the same safety rules as single-agent linking

#### Scenario: Force replaces safe existing link
- **WHEN** the user runs `promptr link --force opencode` and a symlink already exists at the target path
